...
class KeibaApp:
    ...
    # ------ ファイル選択 --------------------------------------
    def _drw_file(self):
        pyxel.rect(0,TOP_MARGIN, W, 44, COL_BG)
        draw_rectb(0,TOP_MARGIN, W, 44, COL_BORDER)
        draw_text(6,  3+TOP_MARGIN, "★ KEIBA ANALYZER ★",                     COL_TITLE,  FONT_M)
        draw_text(6, 20+TOP_MARGIN, "DATAフォルダ: " + truncate(self.data_dir, W-80), COL_HEADER, FONT_S)
    ...
    def _drw_list(self):
        ...
        pyxel.rect(0, TOP_MARGIN, W, 18, COL_BG)
        draw_rectb(0, TOP_MARGIN, W, 18, COL_BORDER)
        draw_btn(2, 2+TOP_MARGIN, text_px("←レース選択") + 70, "←レース選択")
        ox = text_px("←レース選択") + 70
        draw_text(ox + 20, 3+TOP_MARGIN, truncate(race["race_name"], W - ox - 80), COL_TITLE, FONT_M)
        slbl = SORT_LABELS[self.sort_idx]
        draw_btn(W - text_px(slbl) - 40, 3+TOP_MARGIN, text_px(slbl) + 40, slbl, active=True)
    ...
    def _drw_detail(self):
        ...
        draw_text(ox + 12, 3+TOP_MARGIN, truncate(f"{h['no']}番  {h['name']}", W - ox - 16), COL_TITLE, FONT_M)
