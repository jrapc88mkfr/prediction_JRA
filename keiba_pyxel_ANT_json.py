...
def _cache_url(label):
    """Web版: GitHub Pages上にコミットされた結果キャッシュJSONのURL(同一originなのでCORSに引っかからない)"""
    safe = re.sub(r'[\\/:*?"<>|]', "_", label)
...
class _ResultLoader:
    ...
    ・Web版(GitHub Pages)は netkeiba に直接アクセスできない
      (ブラウザのCORS制限があり、requests/bs4も使えないため)。
      そのため、まず「事前にローカルで取得してリポジトリにコミットした
      結果キャッシュJSON」を同一origin(GitHub Pages上のDATA/_result_cache/)
      から読みに行く。 _AsyncLoader は index.json/race.json と同じ仕組みで
      ローカルなら同期ファイル読み込み、Webならpyodide.http.pyfetchを使う。
    ・ローカル実行時は、キャッシュが無ければ netkeiba へライブ取得しに行き、
      成功したらキャッシュとして保存する(次回以降は再取得しない)。
    """
    def __init__(self):
        self.state  = "idle"     # idle / loading / done / error / none
        self.result = None
        self._cache_loader = None   # _AsyncLoader (キャッシュJSON読み込み用)
    ...
    def start(self, data_dir, label):
        ...
        self.result = None
        self._thread = None
        self._cache_loader = _AsyncLoader()
        if IS_WEB:
            self._cache_loader.fetch(_cache_url(label))
        else:
            self._cache_loader.fetch(_cache_path(data_dir, label))

    def poll(self, data_dir, label):
        """毎フレーム呼ぶ。キャッシュ読み込み→(ローカルのみ)無ければライブ取得、の順に進める"""
        if self._cache_loader is not None:
            if self._cache_loader.is_done():
                self.result = self._cache_loader.result
                self.state  = "done"
                self._cache_loader = None
            elif self._cache_loader.is_error():
                # キャッシュ未生成(まだ誰も取得していない)
                self._cache_loader = None
                if IS_WEB or requests is None:
                    self.state = "none"   # Web版はキャッシュ済みの結果しか出せない
                else:
                    self._start_online_fetch(data_dir, label)
            # is_loading中は何もせず、次フレームまた呼ばれるのを待つ
