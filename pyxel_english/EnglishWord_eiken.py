...
def missed_csv_path_for_player(player):
    ...
    return os.path.join(BASE_DIR, f"missed_words_{safe}.csv")


# --- プレイヤーごとの「復習の連続正解数」ファイルパス ---
def review_stats_path_for_player(player):
    safe = player if player else "unknown"
    return os.path.join(BASE_DIR, f"review_stats_{safe}.csv")
...
def load_ranking(filename=RANKING_CSV):
    rows = []
    if not os.path.exists(filename):
        return rows
    with open(filename, encoding="utf-8") as f:
...
def save_ranking_entry(player, score, filename=RANKING_CSV):
    with open(filename, "a", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
...
def load_ranking_best_per_player(filename=RANKING_CSV):
    best = {}
