...
# 正式名称と通称が文字列としてまったく重ならないケース(略記マッチでは拾えない)
_RACE_ALIASES = {
...
def _race_core(name: str) -> str:
    """
    レース名の"核"となる部分だけを取り出す。
    ・冠名付き("農林水産省賞典 函館記念")は末尾の語だけを見る
    ・「ステークス」⇔「S」のような略記や、末尾の"記念/賞/杯"等を落として比較しやすくする
    (手動でキャッシュファイル名を短縮・変更していても同じレースだと判定できるようにするため)
    """
    n = name
    if " " in n or "　" in n:
        n = re.split(r"[ 　]+", n)[-1]
...
def _race_core_variants(name: str):
    core = _race_core(name)
    variants = {core}
    if core in _RACE_ALIASES:
        variants.add(_race_core(_RACE_ALIASES[core]))
    return variants
...
def _manifest_path(data_dir):
    return os.path.join(data_dir, RESULT_CACHE_SUBDIR, MANIFEST_NAME)


def _manifest_url():
    return f"{BASE_URL}/DATA/{RESULT_CACHE_SUBDIR}/{MANIFEST_NAME}"


def _cache_url_for_label(cache_label):
    safe = re.sub(r'[\\/:*?"<>|]', "_", cache_label)
    return f"{BASE_URL}/DATA/{RESULT_CACHE_SUBDIR}/{safe}_result.json"


def refresh_result_cache_manifest(data_dir):
    """
    _result_cache フォルダの中身を毎回スキャンして _manifest.json を作り直す
    (netkeibaから自動取得したファイルだけでなく、手動で置いた/リネームした
    結果JSONも含めて一覧にする)。ローカル実行時のみ呼ぶ。
    Web版はこのマニフェストを読んで、レース名のゆらぎを許容しつつ
    該当ファイルを探す。
    """
    if IS_WEB:
        return
    try:
        ...
        for fn in os.listdir(d):
            if not fn.endswith("_result.json") or fn == MANIFEST_NAME:
                continue
            labels.append(fn[: -len("_result.json")])
        with open(_manifest_path(data_dir), "w", encoding="utf-8") as f:
            json.dump(sorted(labels), f, ensure_ascii=False, indent=2)
    except Exception as e:
        print(f"[WARN] 結果キャッシュ一覧の作成に失敗: {e}")
...
def save_cached_result(data_dir, label, result):
    ...
    refresh_result_cache_manifest(data_dir)


def _best_manifest_match(manifest_labels, target_key):
    """target_key=(date8,track,race_name) に一番近いキャッシュのラベルを探す
    (日付のズレや「ステークス」⇔「S」等の略記、有名な通称の違いを許容する)"""
    if not target_key:
        return None
    _, track, race_name = target_key
    target_variants = _race_core_variants(race_name)
    best = None
    for cand_label in manifest_labels:
        key = _parse_race_key_from_label(cand_label)
        if not key:
            continue
        _, cand_track, cand_name = key
        if cand_track != track:
            continue
        cand_variants = _race_core_variants(cand_name)
        if any(t and c and (t in c or c in t)
               for t in target_variants for c in cand_variants):
            best = cand_label
            break
    return best
...
class _ResultLoader:
    ...
    ・ローカル実行時: まず data_dir/_result_cache/<label>_result.json を
      同期読み込みし、無ければ netkeiba へライブ取得しに行って保存する。
    ・Web版(GitHub Pages): netkeiba に直接アクセスできない
      (ブラウザのCORS制限があり、requests/bs4も使えないため)。
      事前にローカルで取得し、リポジトリにコミットした結果キャッシュを
      同一origin から読む。ただしファイル名が手動でリネーム・省略される
      ことがあるため、まず _manifest.json (キャッシュフォルダの一覧)を
      読み、レース名のゆらぎを許容してマッチするファイルを探してから
      そのファイルを取得する。
    """
    def __init__(self):
        self.state  = "idle"     # idle / loading / done / error / none
        self.result = None
        self._stage  = None      # 'local' / 'manifest' / 'data'
        self._loader = None      # 現在進行中の _AsyncLoader
        self._thread = None      # ローカルのみ: netkeibaライブ取得スレッド
        self._target_key = None  # (date8, track, race_name)

    def start(self, data_dir, label):
        self.state  = "loading"
        self.result = None
        self._thread = None
        self._target_key = _parse_race_key_from_label(label)
        self._loader = _AsyncLoader()
        if IS_WEB:
            self._stage = "manifest"
            self._loader.fetch(_manifest_url())
        else:
            self._stage = "local"
            self._loader.fetch(_cache_path(data_dir, label))

    def poll(self, data_dir, label):
        """毎フレーム呼ぶ。段階を進めながら結果に辿り着く"""
        ...
        if self._stage == "local":
            if self._loader.is_done():
                ...
                if requests is None:
                    ...
                    self._start_online_fetch(data_dir, label)
                return
            self._loader = None
            return

        if self._stage == "manifest":
            manifest = self._loader.result if self._loader.is_done() else []
            self._loader = None
            match_label = _best_manifest_match(manifest or [], self._target_key)
            if not match_label:
                self.state = "none"
                return
            self._stage  = "data"
            self._loader = _AsyncLoader()
            self._loader.fetch(_cache_url_for_label(match_label))
            return
    ...
    def _start_online_fetch(self, data_dir, label):
        self.state  = "loading"
        self.result = None
    ...
    def update(self):
            ...
            if self.scene == SCENE_FILE:
                ...
                if self.current_race:
                    self._result_loader.poll(self.data_dir, self.current_race["race_name"])
...
if __name__ == "__main__":
    ...
    refresh_result_cache_manifest(data_dir)  # 手動で置いた/リネームしたキャッシュも一覧に含める
    KeibaApp(data_dir)
