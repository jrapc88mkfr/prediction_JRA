...
def _find_font(size: int) -> ImageFont.FreeTypeFont:
    """
    Windows/Mac/Linux の標準日本語フォントを探してロードする。
    見つからなければ同フォルダの font.ttf を試み、
    それもなければ PIL デフォルト(英字のみ)にフォールバック。
    """
    candidates = [
        # Windows
    ...
    for p in candidates:
        if os.path.exists(p):
            try:
                ...
                continue
    print("[WARN] 日本語フォントが見つかりません。英字のみ表示になります。")
...
def _render_text_to_pixels(text: str, size: int, col_idx: int):
    """
    PIL でテキストをレンダリングし、非背景ピクセルを
    [(dx, dy)] のリストとして返す。キャッシュ付き。
    """
    key = (text, size, col_idx)
    ...
    draw.text((-bbox[0] + 1, -bbox[1] + 1), text, font=font, fill=(255, 255, 255))
...
def draw_text(x: int, y: int, text: str, col: int = COL_TEXT, size: int = FONT_S):
    """Pyxel 画面に日本語対応テキストを描画する"""
    if not text:
        return
    tw, th, pixels = _render_text_to_pixels(str(text), size, col)
    for dx, dy in pixels:
        px, py = x + dx, y + dy
...
def text_px(text: str, size: int = FONT_S) -> int:
    """テキストの描画幅(px)を返す"""
    if not text:
        return 0
    font = get_font(size)
    dummy = Image.new("RGB", (1, 1))
    dd    = ImageDraw.Draw(dummy)
    bbox  = dd.textbbox((0, 0), str(text), font=font)
...
def draw_btn(x, y, w, label, active=False, fsize=FONT_S):
    ...
    pyxel.rect(x, y, w, FONT_S + 4, bg)
    draw_rectb(x, y, w, FONT_S + 4, 7 if active else COL_BORDER)
...
class KeibaApp:
    ...
    def _hdr(self, title: str, sub: str = "", back: str = ""):
        ...
        if sub:
            draw_text(ox, 3 + FONT_M + 1, truncate(sub, W - ox - 4), COL_MUTED, FONT_S)
    ...
    def _drw_detail(self):
        ...
        def oy(dy): return 22 + dy - sc * (FONT_S + 3)

        def line(dy, label, val, vc=COL_TEXT):
            y = oy(dy)
            if not (18 < y < H - 13): return
            draw_text(4, y, label, COL_HEADER, FONT_S)
            draw_text(4 + text_px(label, FONT_S) + 4, y, str(val), vc, FONT_S)

        def section(dy, title_s):
            y = oy(dy)
            if not (18 < y < H - 13): return
            pyxel.rect(2, y, W - 8, FONT_S + 4, COL_BG)
            draw_rectb(2, y, W - 8, FONT_S + 4, COL_BORDER)
