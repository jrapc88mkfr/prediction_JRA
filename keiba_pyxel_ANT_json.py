...
class AsyncLoader:
    ...
    def poll(self):
        """
        Pyodide では asyncio.ensure_future でタスクを投げると
        Pyodide 側が自動でイベントループを進めるので
        run_until_complete は不要（むしろ SuspendError の原因になる）
        ローカル版も何もしない（同期フェッチのため不要）
        """
        pass  # 何もしない
...
def parse_races_from_json(data: dict, label: str) -> list:
    """
    JSON構造に対応:
      "pyxel": [ { "馬番":1, "馬名":"...", "オッズ戦績":"24.9 (4.2.1.12)",
                   "性齢":"北5/鹿", "斤量":"53.0kg", "騎手":"...",
                   "総合指数":72, "前走":80, "前々":96, "3走":89,
                   "前3f":34.5, "調1f":12.3, "印":"" }, ... ]
    """
    pyxel_rows = data.get("pyxel", [])
    if not pyxel_rows:
        print(f"[PARSE] pyxelキーなし or 空  keys={list(data.keys())[:10]}")
        return []
    ...
    for row in pyxel_rows:
        try:
            ...
            # 性齢: "牝5"/"牡4"/"セ6" か "北5/鹿"(産地/毛色) かを判別
            sex_raw = str(row.get("性齢",""))
            if re.search(r"[牝牡セ]", sex_raw):
                sex = sex_raw.split("/")[0].strip()
            else:
                sex = sex_raw  # 産地/毛色形式ならそのまま表示

            # オッズ: "24.9 (4.2.1.12)" → 先頭数値
            odds_str = str(row.get("オッズ戦績","")).strip()
            odds_m = re.match(r"(\d+\.?\d*)", odds_str)
            odds = float(odds_m.group(1)) if odds_m else None

            # 前3F: キー名の揺れに対応
            prev_3f = ""
            for k in ["前3F", "前3f", "前走3F", "前走3f"]:
                v = row.get(k)
                if v is not None:
                    prev_3f = safe(v)
                    break
            ...
            print(f"[WARN] skip row: {ex}  row={str(row)[:80]}")
    ...
    print(f"[PARSE OK] {label}  {len(horses)}頭")
    ...
    def update(self):
        ...
        if self.scene == SCENE_LOADING:
            if self._index_loader.is_done():
                ...
                err = self._index_loader.error
                print(f"[INDEX ERROR] {err}")
                self.err_msg   = f"index取得失敗: {err[:80]}"
                self.file_list = [{"path":"__DUMMY__","label":"サンプルデータ","dir":"."}]
                self.scene     = SCENE_FILE
            return
